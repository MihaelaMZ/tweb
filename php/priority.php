<!DOCTYPE html>
<html lang="ro">

<head>
    <title>UE priority</title>
    <meta charset="utf-8">
    <link rel="icon" type="image/x-icon" href="../images/favicon.png">
    <link rel="stylesheet" href="../css/menu.css">
    <link rel="stylesheet" href="../css/common.css">
</head>

<body>
    <div class="menu">
        <a href="../index.php">UE</a>
        <a href="history.php">Istorie</a>
        <a href="symbols.php">Simboluri</a>
        <a href="countries.php">&#354&#0259ri</a>
        <a class="active" href="priority.php">Priorit&#0259&#355i</a>
        <a href="contact.php">Contact</a>
    </div>

    <h1>Priorit&#0259&#355ile Uniunei Europeane</h1>

    <h2>2019-2024</h2>

    <p>
        Protejarea cetățenilor și a libertăților
        <br>
        Dezvoltarea unei baze economice solide și dinamice
        <br>
        Construirea unei Europe neutre din punct de vedere climatic, verzi,
        echitabile și sociale
        <br>
        Promovarea intereselor și a valorilor europene pe plan mondial
    </p>

    <p>
        <img src="../images/priorityim.png" width="370" height="200">
    </p>
</body>

</html>